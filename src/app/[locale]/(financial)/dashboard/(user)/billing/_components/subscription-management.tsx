    ...
    return (<div className={cn("text-left w-full", className)}>
        <Card className="shadow-lg">
            <CardHeader className="pb-4 sm:pb-6 px-4 sm:px-6">
                ...
                <CardDescription className="text-sm sm:text-base">{t('manageSubscriptionDescription')}</CardDescription>
            </CardHeader>
            ...
            <CardContent className="space-y-6 sm:space-y-8 px-4 sm:px-6">
                            ...
                            <div className="w-full">
                                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-2">
                                    <div className="flex items-center gap-2">
                                        <h3 className="text-lg sm:text-xl font-semibold">{currentPlan.plan.title} {t('plan')}</h3>
                                    </div>
                                    <div className="flex flex-wrap items-center gap-2">
                                        ...
                                        >
                                            {currentPlan.type === `monthly` ? `${currentPlan.plan.currency}${currentPlan.plan.monthlyPrice}/${t('month')}` : currentPlan.type === `yearly` ? `${currentPlan.plan.yearlyPrice}/${t('year')}` : `${currentPlan.price}`}
                                        </Badge>
                                        <Badge
                                            variant="outline"
                ...
                <div className="space-y-3 sm:space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6">
                        <div className="group p-2.5 sm:p-3 rounded-lg md:bg-gradient-to-tl bg-gradient-to-b from-muted to-background/10 border border-border/30 hover:border-border/60 transition-all duration-200">
                            <span className="text-xs sm:text-sm text-muted-foreground block mb-1">{t('nextBillingDate')}</span>
                        ...
                        <div className="group p-2.5 sm:p-3 rounded-lg md:bg-gradient-to-tr bg-gradient-to-b from-muted to-background/10 border border-border/30 hover:border-border/60 transition-all duration-200">
                            <span className="text-xs sm:text-sm text-muted-foreground block mb-1">{t('paymentMethod')}</span>
                ...
                <div className="pt-4 sm:pt-6">
                    <h4 className="font-medium mb-3 sm:mb-4 text-base sm:text-lg">{t('currentPlanFeatures')}</h4>
