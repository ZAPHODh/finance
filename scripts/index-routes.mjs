...
async function indexRoutes() {
  ...
  if (!appId || !writeKey) {
    console.error('Missing Algolia credentials. Please check your environment variables.');
    console.error('Required: NEXT_PUBLIC_ALGOLIA_APP_ID, ALGOLIA_WRITE_KEY');
    process.exit(1);
  }

  console.log('Indexing routes to Algolia...');
  ...
  try {
    ...
    console.log(`✅ Successfully indexed ${routes.length} routes to Algolia index: ${INDEX_NAME}`);
  } catch (error) {
    console.error('❌ Error indexing routes:', error);
    process.exit(1);
  }
}
